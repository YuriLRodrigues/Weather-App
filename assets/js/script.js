...
searchBtn.addEventListener('click', async () => {
  const APIKey = '8b0d9feff430d6b8cd213c1af90adeb0';
  const city = document.querySelector('.search-box input').value

  if (city === ''){
    return;
  }
  await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${APIKey}`).then(res => res.json()).then(
    json => {
      console.log(json)
      if (json.cod === '404'){
